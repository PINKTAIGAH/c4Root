...
#include "FatimaV1751Trace8BitData.h"

#include "TClonesArray.h"
#include "TMath.h"
#include "v1751DpppsdTrace8BitRaw2Cal.h"

/*
Empty constructor for FairRoot.
*/
v1751DpppsdTrace8BitRaw2Cal::v1751DpppsdTrace8BitRaw2Cal()
  : FairTask(), 
  fNEvents(0),
  header(nullptr),
  fOnline(kFALSE),
  funcal_data(new TClonesArray("v1751DpppsdTrace8BitData")),
  fcal_data(new TClonesArray("FatimaV1751Trace8BitData"))
  {
...
*/
v1751DpppsdTrace8BitRaw2Cal::v1751DpppsdTrace8BitRaw2Cal(const TString& name, Int_t verbose) 
...
v1751DpppsdTrace8BitRaw2Cal::~v1751DpppsdTrace8BitRaw2Cal(){
    c4LOG(info, "Deleting v1751DpppsdTrace8BitRaw2Cal task");
...
void v1751DpppsdTrace8BitRaw2Cal::SetParContainers()
{
...
*/
InitStatus v1751DpppsdTrace8BitRaw2Cal::Init()
{
    ...
    funcal_data = (TClonesArray*)mgr->GetObject("v1751DpppsdTrace8BitData");
    c4LOG_IF(fatal, !funcal_data, "Fatima branch of v1751DpppsdTrace8BitData not found.");
 
    // needs to have the name of the detector subsystem here:
    FairRootManager::Instance()->Register("FatimaV1751Trace8BitData", "Fatima Cal Data", fcal_data, !fOnline);
...
double v1751DpppsdTrace8BitRaw2Cal::Calibrate_QDC_E(double E, int det_id)
{
...
*/
void v1751DpppsdTrace8BitRaw2Cal::Exec(Option_t* option)
{
    if (funcal_data && funcal_data->GetEntriesFast() > 0)
    {
        FatimaV1751Trace8BitData event;
        ...
        {
            funcal_hit = (v1751DpppsdTrace8BitData*)funcal_data->At(ihit);
            ...
            uint8_t digital_probe_1[VMEV1751_TRACE_SIZE] = {0};
            uint8_t digital_probe_2[VMEV1751_TRACE_SIZE] = {0};
            for (int idx = 0; idx < trace_length; idx++){
                trace[idx] = funcal_hit->Get_trace(idx);
                digital_probe_1[idx] = funcal_hit->Get_digital_probe_1(idx);
                digital_probe_2[idx] = funcal_hit->Get_digital_probe_2(idx);
            }
            ...
            for (int idx = 0; idx < trace_length; idx++){
                ...
                event.Set_digital_probe_1_val((uint8_t)digital_probe_1[idx], idx);
                event.Set_digital_probe_2_val((uint8_t)digital_probe_2[idx], idx);
            }
            ...
            if (set_write) new ((*fcal_data)[fcal_data->GetEntriesFast()]) FatimaV1751Trace8BitData(event);  
...
*/
void v1751DpppsdTrace8BitRaw2Cal::FinishEvent(){
    funcal_data->Clear();
    fcal_data->Clear();
};
...
ClassImp(v1751DpppsdTrace8BitRaw2Cal)
